{% set ga_ec_beg_date = ga_ec_beg_date if target.name == 'prd' else (modules.datetime.datetime.now() - modules.datetime.timedelta(days=7)).isoformat() %}
...
FROM {{ source('CA_GOOGLE_ANALYTICS', 'ANALYTICS_471611234__VIEW') }}
